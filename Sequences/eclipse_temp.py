...
# ~ separation between the Moon and the Sun is more than 180deg-0.9deg (= 179.1deg).
# ~ The number 0.9deg is hardcoded for simplicity, for more accuracy, it
...
while curtime <= endtime:
    ...
    # it from radians to degrees, subtract it by 180deg.
    # this is basically the separation of the moon from the Earth's
    # center of umbra.
    sep = abs((float(ephem.separation(moon, sun))
        / 0.01745329252) )

    # eclipse occurs if the separation is less than 0.9deg.
    ...
    if sep < 1.5:
        print(curtime.strftime('%Y/%m/%d %H:%M:%S'), sep, moon.alt)
